...
export default function AdminOrdersPage() {
    ...
    try {
      if (newStatus === editTarget.status) {
        if (
          ...
          (courierName !== editTarget.courierName ||
            trackingNumber !== editTarget.trackingNumber)
        ) {
          await orderService.startShipping(
            editTarget._id,
            courierName,
            trackingNumber,
          );
        ...
        switch (newStatus) {
          case "processing":
            await orderService.startPreparation(editTarget._id);
            break;
          case "shipped":
            await orderService.startShipping(
              editTarget._id,
            ...
            break;
          case "delivered":
            await orderService.completeDelivery(editTarget._id);
            break;
          case "cancelled":
            await orderService.cancelOrder(editTarget._id);
            break;
        }
        toast.success(
          `주문 ${editTarget.orderNumber} 상태를 "${STATUS_LABEL[newStatus]}"로 변경했습니다.`,
        );
      }
      setEditTarget(null);
      await fetchOrders();
  ...
  const allPaid =
    ...
    selectedOrders.every((o) => o.status === "paid");
  const allProcessing =
    selectedOrders.length > 0 &&
    selectedOrders.every((o) => o.status === "processing");
  const allShipped =
    selectedOrders.length > 0 &&
    selectedOrders.every((o) => o.status === "shipped");
  ...
  return (
    <>
        ...
        <div className="mb-6 grid grid-cols-3 gap-3 lg:grid-cols-6">
          {ALL_STATUSES.map((s) => (
            ...
            >
              ...
              <p
                className={`mt-1 text-2xl font-bold ${filterStatus === s ? "text-brand-blue" : "text-foreground"}`}
              >
                {statusCounts[s]}
              </p>
            </button>
      ...
      <Dialog
        open={bulkShipOpen}
        onOpenChange={(open) => !open && setBulkShipOpen(false)}
      >
        <DialogContent>
          <DialogHeader>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setBulkShipOpen(false)}>
              취소
            </Button>
            <Button onClick={handleBulkShip} disabled={isSubmitting}>
              배송 시작
            </Button>
          </DialogFooter>
        </DialogContent>
